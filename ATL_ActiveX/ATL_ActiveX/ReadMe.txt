========================================================================
ATL (ACTIVE TEMPLATE LIBRARY) : ATL_ActiveX プロジェクトの概要
========================================================================

この ATL_ActiveX プロジェクトは、ユーザーがダイナミック リンク ライ
...
ATL_ActiveX.idl
    ...
    このファイルは MIDL コンパイラによって処理され、次のものが生成されます。
        C++ のインターフェイス定義と GUID 宣言 (ATL_ActiveX.h)
        GUID 定義                              (ATL_ActiveX_i.c)
        タイプ ライブラリ                      (ATL_ActiveX.tlb)
        マーシャリング コード                  (ATL_ActiveX_p.c および dlldata.c)

ATL_ActiveX.h
    このファイルには、C++ のインターフェイス定義および 
    ATL_ActiveX.idl. で定義される項目の GUID 宣言が含まれます。
...
StdAfx.h, StdAfx.cpp
    ...
    ATL_ActiveX.pch とプリコンパイル済み型ファイル StdAfx.obj 
    をビルドするために使用します。
...
ATL_ActiveXps.vcproj
	...
	ATL_ActiveX_i.c と ATL_ActiveX_p.c が生成され
        ます。
