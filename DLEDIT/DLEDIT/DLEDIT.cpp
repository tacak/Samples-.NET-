// DLEDIT.cpp : コンソール アプリケーションのエントリ ポイントを定義します。
//
...
void _tmain (int argc, TCHAR *argv[])
{
   ...
   else
   {
      ...
      // SetVolumeMountPoint. This requires the unique volume name 
      // of the device to which the new drive letter will refer. 
      // To get the unique volume name, use 
      // GetVolumeNameForVolumeMountPoint; it requires the drive 
      // letter to already exist. So, we first define the drive 
      // letter as a symbolic link to the device name. After we 
      // have the unique volume name the new drive letter will 
      // point to, we must delete the symbolic link because the 
...
void PrintHelp (LPCTSTR pszAppName)
{
   ...
   _tprintf(TEXT("Usage: %s <Drive> <Device name>   add a drive fletter\n"), pszAppName);
